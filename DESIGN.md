...
**当前实现（ME Federation Pattern Provider，`ae2federation:pattern_provider`）：** 方块朝向（AE2 facing 策略，可用扳手旋转）即 Federation 面，放置时贴向所点击的方块；另外五个面暴露同一个原生 ME 节点。9 个物理 Pattern 槽位由一个不执行的原生 `PatternProviderLogic` 持有，因此 AE2 自己的样板供应器界面、样板访问终端、内存卡与掉落规则直接作用于它；Blocking、Lock Crafting、优先级等设置逐一写入每条 Lane 自己的原生配置。在联邦域管理界面把某个槽位映射到某个 Endpoint 时，服务端为该 Provider 获取 Endpoint Claim，并为每个 Endpoint 分配一条原生 Lane（Lane 数量随映射而定，不是固定上限）；每条 Lane 作为独立的原生 crafting provider 发布，由 AE2 规划器与 CPU 选择。取消映射后，Lane 立即停止发布新工作，但返回归属与 Claim 分开处理：原生 send/return 为空并不能证明机器内已无待返回产物，因此一条 Lane 自上次（重新）绑定以来只要成功推送过一次原生 Pattern（`dispatched` 标记，随 Provider 保存），取消最后一个映射后仍保留 Claim 与返回绑定，联邦域界面显示“已保留，可能仍有产物返回”；从未推送过的空闲 Lane 在 send/return 清空后自动释放。被保留的 Endpoint 只能由玩家在映射面板按两次“释放保留”显式释放（第一次显示后果：机器内尚未返回的产物将不再返回此 Provider，等待它们的合成任务需要手动取消），之后可被其他 Provider 重新 Claim。Endpoint 或 Provider 单独重载后，Provider 的维护步骤（每 20 tick、只遍历自身 Lane）在 Endpoint 身份仍匹配而返回未绑定时，经完整授权解析（Provider、Endpoint、Claim Epoch、Lane、Policy）重新绑定返回，即使原生锁仍持有也无需打开 GUI 或新任务；被替换的 Endpoint、变更的 Claim 或撤销的 Policy 不会被重新绑定。拆除方块时 Pattern 只掉落一次、Lane 的待发送/返回资源按原生 `addDrops` 掉落并释放 Claim（若 Endpoint 未加载，则在其加载时释放）。已绑定 Lane 不会把输入交给相邻的 `ICraftingMachine` 或本地库存。
