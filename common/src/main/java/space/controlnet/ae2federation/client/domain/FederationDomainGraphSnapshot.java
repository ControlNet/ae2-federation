...
public record FederationDomainGraphSnapshot(long topologyRevision, long dataRevision, List<Node> nodes, List<Edge> edges,
        List<String> patterns) {
    private static final int WIRE_VERSION = 1;
    private static final int MAX_ENTRIES = 512;

    public FederationDomainGraphSnapshot {
    ...
    public static FederationDomainGraphSnapshot empty() {
        return new FederationDomainGraphSnapshot(0, 0, List.of(), List.of(), List.of());
    }
    ...
    public static FederationDomainGraphSnapshot decode(String encoded) {
        ...
        if (header.length != 3 || Integer.parseInt(header[0]) != WIRE_VERSION) {
            throw new IllegalArgumentException("Unsupported Federation Domain graph wire version");
        }
        ...
        for (var index = 1; index < lines.size(); index++) {
            ...
            switch (fields[0]) {
                case "N" -> {
                    ...
                    nodes.add(new Node(decoded(fields[2]), FederationDomainGraphNodeKind.valueOf(fields[1]), decoded(fields[3])));
                }
                case "E" -> {
                    requireFields(fields, 5);
                    edges.add(new Edge(decoded(fields[2]), decoded(fields[3]), FederationDomainGraphLayer.valueOf(fields[1]),
                            decoded(fields[4])));
                }
                case "P" -> {
                ...
                default -> throw new IllegalArgumentException("Unknown Federation Domain graph wire entry");
            }
        }
        return new FederationDomainGraphSnapshot(Long.parseLong(header[1]), Long.parseLong(header[2]), nodes, edges, patterns);
    }
    ...
    private static List<String> canonicalPatterns(List<String> values) {
        ...
        values.forEach(FederationDomainGraphSnapshot::requireText);
    ...
    private static void requireFields(String[] fields, int expected) {
        if (fields.length != expected) {
            throw new IllegalArgumentException("Malformed Federation Domain graph wire entry");
        }
    }
    ...
    private static void requireText(String value) {
        ...
        if (value.length() > 512) {
            throw new IllegalArgumentException("Federation Domain graph text is too long");
        }
    }

    public record Node(String id, FederationDomainGraphNodeKind kind, String status) {
        public Node {
            requireText(id);
    ...
    public record Edge(String from, String to, FederationDomainGraphLayer layer, String status) {
        public Edge {
            requireText(from);
            requireText(to);
