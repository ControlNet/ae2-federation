...
final class FederationDomainPolicyObservation implements ObservationAuthority {
    private final FederationDomainPolicySession session;
    private final ServerPlayer player;
    private final ServerLevel level;
    private final FederationDomainReference scope;
    private final UUID sessionId = UUID.randomUUID();

    FederationDomainPolicyObservation(FederationDomainPolicySession session, ServerPlayer player, FederationDomainReference scope) {
    ...
    public boolean current() {
        return session.isStillValid(player) && FederationDomainRegistryAccess.get(level).isCurrent(scope);
    }
