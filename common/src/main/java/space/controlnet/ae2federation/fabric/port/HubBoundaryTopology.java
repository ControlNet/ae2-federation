...
public final class HubBoundaryTopology {
    private final Map<Direction, NativeAttachment> attachments;
    private final Map<IGrid, List<Direction>> facesByGrid;

    private HubBoundaryTopology(Map<Direction, NativeAttachment> attachments, Map<IGrid, List<Direction>> facesByGrid) {
        this.attachments = Collections.unmodifiableMap(attachments);
        this.facesByGrid = Collections.unmodifiableMap(facesByGrid);
    }

    public static Optional<HubBoundaryTopology> resolve(Level level, Map<Direction, BoundaryPort> ports) {
        ...
        return Optional.of(new HubBoundaryTopology(attachments, facesByGrid));
    }
    ...
    public boolean permitsNativeJoin(Direction first, Direction second) {
        if (!attachments.containsKey(first) || !attachments.containsKey(second)) {
            throw new IllegalArgumentException("Both Hub faces must be attached");
        }
        return false;
    }
