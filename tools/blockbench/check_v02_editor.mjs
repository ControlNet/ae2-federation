...
const folder=path.resolve('tools/blockbench/versions/v02-ae2-ceramic');const client=new BlockbenchMcp();await client.init();
const evidence=path.resolve('tools/blockbench/.local/verification/v02-editor');fs.mkdirSync(evidence,{recursive:true});
const lit=v=>JSON.stringify(v).replaceAll('/', '\\u002f');
const value=r=>r.structuredContent??JSON.parse(r.content.find(c=>c.type==='text').text);
const results=[];
try{
 const files=fs.readdirSync(path.join(folder,'models')).filter(f=>f.endsWith('.bbmodel')).sort((a,b)=>Number(a==='v02_family.bbmodel')-Number(b==='v02_family.bbmodel'));
