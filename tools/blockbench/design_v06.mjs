/** V06: AE2 quartz edge and flat functional glyphs, painted through native MCP. */
import fs from 'node:fs';import path from 'node:path';import {BlockbenchMcp} from './mcp_client.mjs';
const root=path.resolve(import.meta.dirname,'../..');
const base=path.join(root,'tools/blockbench/versions/v04-ae2-quartz');
const work=path.join(root,'tools/blockbench/.local/v06-work');
...
const tiles={};
// Continuous colored substrates fill the native AE2 10x10 functional field.
// Features are changes within the material, not isolated icons on white space.
tiles.router=glyph([
 'aaabbbbaaa','abbaccabba','abacbbcaba','bacaaaacab',
 'bcbabbabcb','bcbabbabcb','bacaaaacab','abacbbcaba',
 'abbaccabba','aaabbbbaaa'],C);
// Three pattern layers share one full-width substrate and a distribution edge.
tiles.pattern_provider=glyph([
 'abbbbbbbba','acccccccba','aaaaaaaaaa','aabbbbbbba',
 'aaccccccba','aaaaaaaaaa','abbbbbbbba','acccccccba',
 'aaaaaaaaaa','ccaaaacccc'],C);
// A broad processing window occupies a solid active panel, with compact stops.
tiles.processing_endpoint=glyph([
 'aaaabbbaaa','abbbbbbbba','abaaaaaaba','abaccccaba',
 'abacbbcaba','abacbbcaba','abaccccaba','abaaaaaaba',
 'acccaaccca','aaaaccaaaa'],C);
// ME surfaces use a full Fluix substrate with interlocking plate segments.
tiles.me_port=glyph([
 'aaabbbbaaa','abbbbbbbba','abaccccaba','abacaaaaba',
 'abacabbaba','abaccababa','abaaaababa','abccccbaba',
 'abbbbbbbba','aaabbbbaaa'],M);
let side=glyph([
 'aaabbbbaaa','abbbbbbbba','abaccccaba','abacaaaaba',
 'abacabbaba','abaccababa','abaaaababa','abccccbaba',
 'abbbbbbbba','aaabbbbaaa'],M);
// Only this short cue changes identity; the rest of each side is a ME surface.
