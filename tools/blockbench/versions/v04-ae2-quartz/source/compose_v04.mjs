/** Compose V04 review scenes without changing the individual device projects. */
import fs from 'node:fs';import path from 'node:path';import crypto from 'node:crypto';
const work=path.resolve('tools/blockbench/.local/v04-work');
const base=path.resolve('tools/blockbench/versions/v02-ae2-ceramic');
...
for(const name of ['router','pattern_provider','processing_endpoint'])save(name+'_comparison',[
 ['V03',JSON.parse(fs.readFileSync(path.resolve('tools/blockbench/versions/v03-ae2-blocks/models',name+'.bbmodel'))),[0,0,0]],['V04',read(name),[24,0,0]]]);
save('v04_key_faces',[['Router',read('router'),[0,0,0]],['Provider',read('pattern_provider'),[24,0,0]],['Endpoint',read('processing_endpoint'),[48,0,0]]]);
const family=read('v02_family',true);family.name='v04_family';
...
fs.writeFileSync(path.join(work,'models/v04_family.bbmodel'),JSON.stringify(family,null,2)+'\n');
...
const context=scene('AE2 19.2.17 context',[
 ['AE2 Provider',native('pattern_provider'),[0,0,0]],['V04 Provider',read('pattern_provider'),[24,0,0]],
 ['AE2 Interface',native('interface'),[48,0,0]],['V04 Endpoint',read('processing_endpoint'),[72,0,0]]]);
fs.writeFileSync(path.join(work,'ae2_context.bbmodel'),JSON.stringify(context,null,2)+'\n');
console.log('Composed V03/V04 comparisons, family, key faces and local AE2 context.');
