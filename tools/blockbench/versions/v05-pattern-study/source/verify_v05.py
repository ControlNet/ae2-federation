"""Verify V05 painted pixels, unchanged geometry and preserved V02 devices.
...
folder = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / 'tools/blockbench/.local/v05-work'
...
print(f'V05: {len(tiles)} painted tiles match; block geometry/UVs unchanged; Router faces equivalent; {kept} Bridge/cable projects and all unrelated textures byte-identical to V02.')
# V04's approved quartz background and outline are outside the 10x10 glyph area.
...
print('V05: approved V04 quartz background/outline outside the glyph area preserved on all 8 tiles.')
