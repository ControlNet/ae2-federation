/** Read-only V03 editor verification; evidence never overwrites archived assets. */
import fs from 'node:fs';import path from 'node:path';import {BlockbenchMcp} from './mcp_client.mjs';
const folder=path.resolve(process.argv[2]??'tools/blockbench/.local/v03-work');const client=new BlockbenchMcp();await client.init();
const evidence=path.resolve('tools/blockbench/.local/verification/v03-editor');fs.mkdirSync(evidence,{recursive:true});
const lit=v=>JSON.stringify(v).replaceAll('/', '\\u002f');
const value=r=>r.structuredContent??JSON.parse(r.content.find(c=>c.type==='text').text);
const results=[];
try{
 const files=fs.readdirSync(path.join(folder,'models')).filter(f=>f.endsWith('.bbmodel')).sort((a,b)=>Number(a==='v03_family.bbmodel')-Number(b==='v03_family.bbmodel'));
