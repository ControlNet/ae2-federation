...
An opt-in [standard-shader cable flow prototype](cable-flow-prototype/README.md)
is available through `/ae2f_cable_preview on` (and `off`). It retains the V07
enclosure and substitutes a BER interior in world rendering only. The generator
also creates its separate `textures/entity/cable_flow.png`; the V07 source and
generated block/item models remain unchanged. The implementation descriptions
and V07 evidence below describe the default appearance, with preview disabled.
