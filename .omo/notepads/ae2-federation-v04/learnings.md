...
## 2026-09-25 Task 37 server-thread CPU stage extension

- Gate `ThreadMXBean` lookup behind the timed replay's existing opt-in stage diagnostics; read only supported/enabled current-thread CPU time. Capture at the existing start/transition/report boundaries; if the clock is unavailable, returns negative, or the calling thread changes, label every stage CPU aggregate `unavailable` instead of silently attributing partial intervals. Wall/ticks/waits and the failed warmup exception remain unchanged. Controlled JUnit and strict `test check build` pass, but no real CPU stage data exists until a new authorized run. Exact local gate and limitation: `.omo/evidence/task-37-small-timed-first/verification.md`.
