...
- Task 20 freezes only correctness and resource budgets in `tests/benchmarks/budgets.json`; timing remains `environment-sensitive-secondary` evidence and must not freeze reference medians or define any timing regression trigger, because observed measurements on a shared unpinned machine vary by 19-29% run to run, far beyond any useful threshold. Under the 2026-09-24 user override, Task 37 may record repeated best-effort reserved-VM timing medians per fixed scene/hardware as indicative order-of-magnitude references, not controlled speedup or a strict timing budget. Record CPU model, core count, JVM/flags, affinity, load and competing processes with each sample; document KVM governor unreadability and residual interference. No exclusive cpuset, readable governor or runnable<=2 gate applies. The proposed >10% throughput, p95/p99 MSPT and retained-heap triggers require a genuinely comparable, stable baseline before use; an indicative VM median alone cannot activate them. No resource/identity correctness regression is allowed at any tier, including task 20. Compare to native layouts for attribution, not an unsupported promise of native-equal cost.
...
- [~] 37. Execute small/late/ultra three-layout benchmarks and fix measured regressions
  - Historical blocker (2026-09-23), superseded for host eligibility on 2026-09-24: the absence of a quiesced, pinned baseline machine no longer bars indicative measurements on the reserved VM. The 405-minute minimum serial windows still require sufficient reservation time. Development correctness replays are not the declared timed profiles; no timing run or median is claimed here.
  - Work: generate comparable single-big-grid, native-subnet and Federation layouts; run D T-S01..10 with fixed profiles, same CPUs/energy/loaded chunks/input replay where expressible. Correct measured bottlenecks with focused changes only and rerun affected native correctness/call-trace cases. Record resource mismatches and mark intrinsically or materially mismatched layouts non-comparable for strict speedup. Descriptive observed elapsed ratios are allowed with explicit resource and host caveats. On the reserved VM, collect repeated 300-second warmup/600-second sample measurements and per-sample JVM/CPU/affinity/load/competing-process observations; seek best-effort quiet conditions without claiming zero interference. Label any median an indicative order-of-magnitude reference, not a controlled baseline or automatic >10% regression threshold.
  - References: D19.9-19.13; frozen task 20 correctness/resource budgets; tasks 24,30,36. Depends on 20,24,30,36.
  - Acceptance: raw metrics + conserved work/resource accounts + three repetitions per measured profile and layout + profiles saved; support scale based on observed results. No guaranteed 512-Grid TPS claim. Any budget adjustment includes reason and before/after evidence rather than quietly making a failing run pass. A reserved, best-effort quiet VM is eligible for indicative repeated timing even when KVM governors cannot be read, CPUs lack an exclusive cpuset or runnable count exceeds 2. Log observed competing processes, including `wakatime-cli`, `videnoa` and `herdr`, as interference disclaimers rather than automatic failures. Match and disclose recorded environments for any indicative median; do not claim strict speedup from materially mismatched resource layouts.
...
- [~] 38. Run long-soak churn, restart and uncertainty-boundary qualification
  - Task 37's incomplete scale/profile and resource evidence still blocks final dependent qualification. The 2026-09-24 host override permits a genuine two-hour soak on the reserved best-effort VM once its actual workload and evidence are ready; it does not convert diagnostic work into an acceptance run.
  ...
  - Acceptance: below-capacity backlog remains bounded; cleanup returns to explained baseline; crash uncertainty never causes automatic whole-batch replay; restart retains inactive Policy and native state. Native crash durability limits explicitly reported, not claimed exactly-once. Record JVM/CPU/affinity/load and competing-process observations with each repeated timing sample; label the VM hardware, unreadable KVM governor and residual interference as limitations, not host failures. No exclusive cpuset or runnable<=2 requirement applies, and best-effort reservation does not prove zero interference.
...
- [x] 39. Verify final JAR provenance, supported tuple and reproducible clean-environment build
  - EULA consent received from the user in this session (2026-09-23: “Task39:同意EULA授权”). This authorizes only Task 39's task-owned, loopback-only temporary server/client verification. Record the consent and create an approval file in task-owned evidence before the fresh run; do not reuse the earlier unapproved result. Static archive/clean-build checks remain partial until source-bound same-JAR runtime and persisted verification pass.
...
- [~] 40. Publish local documentation, evidence coverage and honest prototype support boundaries
  - Blocked for final publication by the incomplete Task 37 and Task 38 qualification evidence; interim documentation must label those gates unsupported rather than infer scale or soak support.
...
- [~] F1. Plan compliance audit
  - Blocked (2026-09-24): read-only preflight returned `VERDICT: REJECT`; Tasks 37, 38 and 40 remain `[~]`, the acceptance matrix and required audit cases/evidence are absent. Rerun only after the implementation and external qualification gates pass; do not treat the preflight as the F1 audit.
...
- [~] F2. Code quality review
  - Blocked by the final-wave prerequisite at line 506: Tasks 37, 38 and 40 remain incomplete. No F2 approval or final command/evidence claim is made.
...
- [~] F3. Real manual QA
  - Blocked by the final-wave prerequisite at line 506 and missing Task 37 benchmark report. No client workflow, screenshots or F3 approval is claimed.
...
- [~] F4. Scope fidelity
  - Blocked by the final-wave prerequisite at line 506: the final documentation and qualification evidence are not complete. No F4 approval is claimed.
